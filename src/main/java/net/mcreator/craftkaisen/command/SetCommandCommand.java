...
public class SetCommandCommand {
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("craftKaisen").requires(s -> s.hasPermission(2)).then(Commands.literal("technique").then(Commands.argument("name", EntityArgument.player()).then(Commands.literal("Limitless"))))
